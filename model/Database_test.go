...
func Test_fetchFromSQLite(t *testing.T) {
	...
	blockRange, err := fetchFromSQLite[*BlockRange](sqlite)
	assert.NoError(t, err)
	...
	bookmark, err := fetchFromSQLite[*Bookmark](sqlite)
	assert.NoError(t, err)
	...
	inputField, err := fetchFromSQLite[*InputField](sqlite)
	assert.NoError(t, err)
	assert.Len(t, inputField, 4)
	assert.Equal(t, &InputField{8, "tt71", "First other..", 3}, inputField[3])

	location, err := fetchFromSQLite[*Location](sqlite)
	assert.NoError(t, err)
	...
	note, err := fetchFromSQLite[*Note](sqlite)
	assert.NoError(t, err)
	assert.Len(t, note, 3)
	...
	tag, err := fetchFromSQLite[*Tag](sqlite)
	assert.NoError(t, err)
	assert.Len(t, tag, 3)
	assert.Equal(t, &Tag{2, 1, "Strengthening"}, tag[2])

	tagMap, err := fetchFromSQLite[*TagMap](sqlite)
	assert.NoError(t, err)
	...
	userMark, err := fetchFromSQLite[*UserMark](sqlite)
	assert.NoError(t, err)
